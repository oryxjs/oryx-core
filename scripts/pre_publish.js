...
function main() {
  ...
  fs.readdirSync(`${process.cwd()}/packages`)
    ...
    .map((p) => {
      try {
        const package = require(`${process.cwd()}/packages/${p}/package.json`)
        packages_with_versions[`${package.name}`] = package.version
      } catch (error) {}
    })
  ...
  try {
    ...
    if(!Object.keys(changed_packages).length) {
      ...
      execSync(`yarn up ${bumps}`)
      // update new versions of packages
      fs.writeFileSync(`${process.cwd()}/packages-versions.json`, JSON.stringify(packages_with_versions))
    }
  } catch (error) {
    fs.writeFileSync(`${process.cwd()}/packages-versions.json`, JSON.stringify(packages_with_versions))
