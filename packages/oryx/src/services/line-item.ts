...
import { PricingService, ProductService, RegionService, ProductVariantService } from "./index"
...
class LineItemService extends BaseService {
  ...
  async retrieve(id: string, config = {}): Promise<LineItem | never> {
    const manager = this.manager_
    const lineItemRepository = manager.getCustomRepository(this.lineItemRepository_)
    ...
    if (!lineItem) {
      throw new MedusaError(MedusaError.Types.NOT_FOUND, `Line item with ${id} was not found`)
    }
  ...
  async createReturnLines(returnId: string, cartId: string): Promise<LineItem[]> {
    return await this.atomicPhase_(async (transactionManager: EntityManager) => {
      const lineItemRepo = transactionManager.getCustomRepository(this.lineItemRepository_)

      const itemTaxLineRepo = transactionManager.getCustomRepository(this.itemTaxLineRepo_)

      const returnLineItems = await lineItemRepo.findByReturn(returnId).then((lineItems) => {
  ...
  async generate(
    ...
    return await this.atomicPhase_(async (transactionManager: EntityManager) => {
      const [variant, region] = await Promise.all([
        this.productVariantService_.withTransaction(transactionManager).retrieve(variantId, {
          relations: ["product"],
        }),
        this.regionService_.withTransaction(transactionManager).retrieve(regionId),
      ])
  ...
  async create(data: Partial<LineItem>): Promise<LineItem> {
    return await this.atomicPhase_(async (transactionManager: EntityManager) => {
      const lineItemRepository = transactionManager.getCustomRepository(this.lineItemRepository_)
